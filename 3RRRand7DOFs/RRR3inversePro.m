function [theta,psi] = RRR3inversePro(Q)
...
for i=1:3
    ...
    % 再绕自身z轴转(-pi/2)，到达基座旋转关节初始位置
    % eta_i=(i-1)*2/3*pi,i=1,2,3
    Q_w{i}=Q*rotz(pi/2+eta(i))*roty(beta)*floor(rotz(-pi/2));
    w(:,i)=Q_w{i}*w_0;
    % 反解theta
    % 求U,V,W
    w_x=w(1,i);
    w_y=w(2,i);
    w_z=w(3,i);
    U=-sin(alpha_1)*sin(eta(i))*w_y-sin(alpha_1)*cos(eta(i))*w_x;
    V=sin(alpha_1)*cos(gamma)*cos(eta(i))*w_y-sin(alpha_1)*cos(gamma)*sin(eta(i))*w_x+sin(alpha_1)*sin(gamma)*w_z;
    W=w_y*cos(alpha_1)*sin(gamma)*cos(eta(i))-w_x*cos(alpha_1)*sin(eta(i))*sin(gamma)-w_z*cos(alpha_1)*cos(gamma)-cos(alpha_2);
    % 求A,B,C
    A=W-U;
    B=2*V;
    C=W+U;
